...
void store_cell(int row, int column, const std::string& value) {
    ...
    if (value.empty()) cell_store.erase(key);
    else cell_store[key] = value;
}
...
void commit_visible_cells() {
    ...
    for (int visible_row = 0; visible_row < GRID_CONTROL_ROWS; ++visible_row) {
...
void repaint_grid_cells() {
    ...
    for (const auto& row : data_cells)
        for (HWND cell : row)
            InvalidateRect(cell, nullptr, TRUE);
}
...
void draw_input_file_item(const DRAWITEMSTRUCT& item) {
    ...
    if (supported_input_file(std::filesystem::path(filename))) {
        ...
        if (shown < headers.size() && left + 52 <= right_limit) {
            ...
            HBRUSH badge_brush = CreateSolidBrush(RGB(255, 230, 230));
            FillRect(item.hDC, &badge, badge_brush);
            DeleteObject(badge_brush);
            FrameRect(item.hDC, &badge,
                static_cast<HBRUSH>(GetStockObject(GRAY_BRUSH)));
            SetTextColor(item.hDC, RGB(150, 25, 25));
            DrawTextW(item.hDC, L"Headers unavailable", -1, &badge,
                DT_SINGLELINE | DT_CENTER | DT_VCENTER | DT_NOPREFIX);
        }
    }
...
void remove_selected_input_file() {
    ...
    if (failed_count)
        MessageBoxW(main_window,
            (std::to_wstring(failed_count) +
             L" selected file(s) could not be removed.").c_str(),
    ...
    try {
        ...
        if (change_mode) {
            ...
            for (std::size_t lookup_index = 0;
                ...
                const auto result =
                    engine->universal_lookup(headers, lookup);
                if (result.rows.empty()) {
                    std::vector<std::string> missing_row(headers.size());
...
void set_input_tab(bool active) {
    ...
    const int ids[] = {ID_HELP, ID_MODE, ID_REFRESH, ID_SEARCH, ID_COPY_ALL,
        ID_EXPORT, ID_FIND_TEXT, ID_FIND_NEXT, ID_GRID_SCROLL,
        ID_GRID_HSCROLL};
...
void set_schedule_tab(bool active) {
    ...
    const int workspace_ids[] = {
        ...
        ID_FIND_TEXT, ID_FIND_NEXT, ID_GRID_SCROLL, ID_GRID_HSCROLL};
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, PWSTR, int show) {
    ...
    chart_class.hIcon = LoadIcon(nullptr, IDI_APPLICATION);
    ...
    synonym_class.hIcon = LoadIcon(nullptr, IDI_APPLICATION);
