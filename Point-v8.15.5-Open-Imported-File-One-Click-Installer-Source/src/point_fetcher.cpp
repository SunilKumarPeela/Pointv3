...
bool validate_download(const std::filesystem::path& path) {
    ...
    if (extension == L".xlsx" || extension == L".xlsm")
        return count >= 2 && bytes[0] == 'P' && bytes[1] == 'K';
    if (extension == L".xls") {
        const unsigned char magic[] = {0xD0, 0xCF, 0x11, 0xE0};
        return count >= 4 && std::equal(std::begin(magic), std::end(magic),
            reinterpret_cast<unsigned char*>(bytes));
    }
    return std::find(bytes, bytes + count, '\0') == bytes + count;
}
...
std::wstring download(const DownloadSchedule& schedule) {
    ...
    output.close();
    if (!validate_download(temporary)) {
        std::error_code ignored;
        std::filesystem::remove(temporary, ignored);
        throw std::runtime_error("Downloaded content is not a valid expected file");
    }
...
void start_fetch(std::size_t index) {
    ...
    fetch_thread = std::thread([index] {
        auto result = std::make_unique<FetchResult>();
        result->index = index;
        try {
            result->message = L"Downloaded: " + download(schedules[index]);
            result->success = true;
...
LRESULT CALLBACK window_proc(HWND window, UINT message, WPARAM wparam, LPARAM lparam) {
    ...
    case WM_COMMAND:
        ...
        case ID_RUN: {
            const int selected = static_cast<int>(SendMessageW(schedule_list,
                LB_GETCURSEL, 0, 0));
            if (selected >= 0) start_fetch(static_cast<std::size_t>(selected));
            return 0;
        }
    ...
    case WM_TIMER:
        if (wparam == TIMER_ID && !fetch_running) {
            ...
            for (std::size_t index = 0; index < schedules.size(); ++index) {
                if (schedules[index].next_run <= now) { start_fetch(index); break; }
            }
        }
        return 0;
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, PWSTR, int show) {
    ...
    root = std::filesystem::path(executable).parent_path() / "Fetcher";
    ...
    type.hIcon = LoadIcon(nullptr, IDI_APPLICATION);
