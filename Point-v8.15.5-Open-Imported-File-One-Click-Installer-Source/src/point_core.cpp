...
const Engine::ExactValueIndex& Engine::exact_value_index(
    ...
    index.reserve(
        std::min<std::size_t>(dataset.rows.size(), 131'072));
...
std::string Engine::indexed_value_key(
    ...
    const auto& dataset = datasets_.at(dataset_id);
    std::string field;
    for (const auto& [candidate, candidate_column] :
    ...
    return normalized_field_value(field, value);
}
            ...
            if (set.headers.empty()) {
                ...
                for (std::size_t i = 0; i < set.headers.size(); ++i) {
                    ...
                    if (set.headers[i].empty() ||
                        set.headers[i].size() > limits_.max_header_bytes)
                        throw std::runtime_error("Blank or oversized header");
...
QueryResult Engine::query(const QueryRequest& request) const {
    ...
    const bool has_unique_seed_column = std::any_of(
        seed_columns.begin(), seed_columns.end(),
        [this](const auto& candidate) {
            return indexed_unique_column(
                candidate.first, candidate.second);
        });
    for (const auto& [d, column] : seed_columns) {
        if (has_unique_seed_column &&
